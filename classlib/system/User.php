...
class User {
    ...
    /**
     *
     * @var String email containing User email address
     */
    protected $email;  
    ...
    public function login($email, $password) {
        //This login function checks the users table for valid login credentials

        $this->userTable=new UserTable($this->db);
        if($this->userTable->validate_login($email, $password, $this->encryptPW)){  //check if the login details match any      
                //query the table for that specific user
                $rs=$this->userTable->getRecordByEmail($email);
