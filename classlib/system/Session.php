 ...
 * <li>If the $_SESSION array is empty it means that no user is logged on so the Session Class initialises the session variables with NULL values.</li>
 *</ul>
 * 
 * 
...
class Session { 
    ...
    /**
     *
     * @var String $email Containing User email 
     */
    private $email;       
    ...
    /**
     *
     * @var array $recipeSteps holds recipe steps temporarily while user is creating a recipe
     */
    private $recipeSteps;
    
    /**
     *
     * @var array $recipeIngredients holds recipe ingredients temporarily while user is creating a recipe
     */
    ...
    public function __construct(){
        ...
        if (isset($_SESSION['loggedin'])){
            ...
            $this->loginTimestamp = $_SESSION['loginTimestamp'];
        }
        else{
            $_SESSION['loggedin'] = FALSE;
            $this->loggedin=FALSE;
            $_SESSION["loginTimestamp"]=null;
        ...
        if (isset($_SESSION['recipeSteps'])) {
            $this->recipeSteps = $_SESSION['recipeSteps'];
        }
        else{
            $_SESSION['recipeSteps'] = [];
            $this->recipeSteps = [];
        }
          
        if (isset($_SESSION['recipeIngredients'])) {
            $this->recipeIngredients = $_SESSION['recipeIngredients'];
        }
        else{
            $_SESSION['recipeIngredients'] = [];
    ...
    /**
     * Sets the user email property and saves in the $_SESSION superglobal array
     * 
     ...
     */
    public function setLoginAttempts($num){$this->loginAttempts=$num;$_SESSION['loginAttempts']=$num;}  
    
    
    /**
     * 
     * Sets the list of recipe steps property and saves in the $_SESSION superglobal array
     * 
     * @param array $steps indexed array of recipe steps. 
     */
    public function setRecipeSteps($steps) {$this->recipeSteps=$steps;$_SESSION['recipeSteps']=$steps;}
    
    
    /**
     * 
     * Sets the list of recipe ingredients property and saves in the $_SESSION superglobal array
     * 
     * @param array $ingredients indexed array of recipe ingredients. 
     */
    public function setRecipeIngredients($ingredients) {$this->recipeIngredients=$ingredients;$_SESSION['recipeIngredients']=$ingredients;}
    ...
    /**
     * Returns the list of steps of the recipe currently being created
     * 
     * @return array
     */
    public function getRecipeSteps(){return $this->recipeSteps ?? [];}
    
    
    /**
     * Returns the list of ingredients for the recipe currently being created
     * 
     * @return array
     */
    public function getRecipeIngredients(){return $this->recipeIngredients ?? [];}
    ...
    public function getDiagnosticInfo(){
            $diagnostic = '<!-- SESSION CLASS PROPERTY SECTION  -->';
                $diagnostic .= '<div class="container-fluid"   style="background-color: #AAAAAA">'; //outer DIV                    
                    ...
                    $diagnostic .= "<tr><td>recipeSteps</td>  <td>".htmlspecialchars(json_encode($this->recipeSteps))."</td></tr>";
                    $diagnostic .= "<tr><td>recipeIngredients</td>  <td>".htmlspecialchars(json_encode($this->recipeIngredients))."</td></tr>";                    
                    $diagnostic .= '</table>';
                    $diagnostic .= '<p><hr>';
                $diagnostic .= '</div>';            
